...
CREATE TABLE IF NOT EXISTS `levya_identity`.`COUNTRY` (
  ...
  `Capital` VARCHAR(25) NULL DEFAULT NULL,
  `MapReference` VARCHAR(50) NULL DEFAULT NULL,
  `NationalitySingular` VARCHAR(35) NULL DEFAULT NULL,
  `NationalityPlural` VARCHAR(35) NULL DEFAULT NULL,
  `Currency` VARCHAR(30) NULL DEFAULT NULL,
  `CurrencyCode` VARCHAR(3) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `levya_identity`.`USER` (
  ...
  CONSTRAINT `fk_USER_COUNTRY1`
    ...
    REFERENCES `levya_identity`.`COUNTRY` (`CountryId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
...
CREATE TABLE IF NOT EXISTS `levya_identity`.`WORK` (
  `BELONG_ID` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `BELONG_FROM` DATETIME NULL,
  `BELONG_TO` DATETIME NULL,
  `BELONG_ISACCEPTED` TINYINT(1) NULL,
  `BELONG_ISLOCKED` TINYINT(1) NULL,
  `USER_USER_ID` INT UNSIGNED NOT NULL,
  `PROJECT_PROJECT_ID` SMALLINT UNSIGNED NOT NULL,
  `POSITION_POSITION_ID` SMALLINT UNSIGNED NOT NULL,
  PRIMARY KEY (`BELONG_ID`, `USER_USER_ID`, `PROJECT_PROJECT_ID`),
  UNIQUE INDEX `BELONG_ID_UNIQUE` (`BELONG_ID` ASC),
  INDEX `fk_BELONG_USER1_idx` (`USER_USER_ID` ASC),
